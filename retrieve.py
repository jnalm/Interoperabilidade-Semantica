...
def can_ped(idpedido, iddoente):
	...
	cur.execute("UPDATE Pedido SET estado = 'CANCELADO' WHERE idPedido = " + str(idpedido))
	db.commit()
...
def gen_report2(doente, pedido):
	report = "MSH|^~\&|A|A|B|B|" + str(pedido["data"]) + "||ORM^O01|A" + str(pedido["data"]) + "51000002533|P|2.5|||AL|\nPID|||" + str(doente["idProcesso"]) + "||" + str(doente["nome"]) + "||" + str(doente["idDoente"]) + "|" + str(doente["sexo"]) + "||||||||||" + str(doente["telefone"]) + "|\nPV1||I|INT||||||||||||||||" + str(pedido["idEpisode"]) + "|\nORC|NW|" + str(pedido["idPedido"]) + "|" + str(pedido["idPedido"]) + "||||||" + str(pedido["data"]) + "|\nOBR|01|" + str(pedido["idPedido"]) + "|" + str(pedido["idPedido"]) + "|" + str(pedido["descricao"]) + "|||||||||||^^^|||CR|RXE||||||30||^^^" + str(pedido["data"]) + "^^0||||||"
	return report

# Relatório Exame Imagiologia Cancelamento Slide 2/10
#def gen_report3(doente, pedido):
#	report = "MSH|^~\&|AIDA|AIDA|PACS|PACS|" + pedido["data"] + "||ORM^O01|A" + pedido["data"] + "51000002533|P|2.5|||AL|\nPID|||" + str(doente["idProcesso"]) + "||" + doente["nome"] + "||" + str(doente["idDoente"]) + "|" + doente["sexo"] + "||||||||||" + doente["telefone"] + "|\nPV1||I|INT||||||||||||||||" + str(pedido["idEpisode"]) + "|\nORC|CA|" + str(pedido["idPedido"]) + "|" + str(pedido["idPedido"]) + "||||||" + pedido["data"] + "|\nOBR|01|" + str(pedido["idPedido"]) + "|" + str(pedido["idPedido"]) + "|" + pedido["descricao"] + "|||||||||||^^^|||CR|RXE||||||||^^^" + pedido["data"] + "^^0||||||"
#	return report
...
for i in range(0,1000):
	...
	if choice == 1:
		...
		print "Indique o ficheiro que quer enviar para a Maquina B:"
		file = raw_input()
		send(file)
	elif choice == 5:
		print "Indique o ficheiro que quer analisar:"
		file = raw_input()
		get_estado(ler(file))
		update_estado()
		lastIdReq = getLastId("pedido")
		temp = escreve(idDoente, lastIdReq)
		lastIdWL = getLastId("worklist")
		insert_report(lastIdWL, lastIdReq, temp)
	elif choice == 6:
